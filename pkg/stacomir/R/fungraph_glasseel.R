...
fungraph_glasseel=function(report_mig,table,time.sequence,taxa,stage,dc=null,silent,color=NULL,color_ope=NULL,...){
  # color=null
  # color calculation
  if (is.null(color)) {
  ...
  dots<-list(...)
  if (!"main"%in%names(dots)) main=gettextf("Glass eels graph %s, %s, %s, %s",dis_commentaire,taxa,stage,annee,domain="R-stacomiR")
  else main=dots[["main"]]
  ...
  if (dim(report_df@data)[1]==0 ) {
	...
	if (sum(report_df@data$per_etat_fonctionnement==1)>0){    
	  rect(   xleft =graphdate(as.Date(report_df@data$per_date_debut[
					  report_df@data$per_etat_fonctionnement==1])), 
		  ybottom=2.1,
		  xright=graphdate(as.Date(report_df@data$per_date_fin[
					  report_df@data$per_etat_fonctionnement==1])),
		  ytop=3, 
  ...
  if (dim(report_dc@data)[1]==0 ) {
	...
	if (sum(report_dc@data$per_etat_fonctionnement==1)>0){ 
	  rect(   xleft =graphdate(as.Date(report_dc@data$per_date_debut[
					  report_dc@data$per_etat_fonctionnement==1])), 
		  ybottom=2.1,
		  xright=graphdate(as.Date(report_dc@data$per_date_fin[
					  report_dc@data$per_etat_fonctionnement==1])),
		  ytop=3, 
		  col = mypalette["working"],
		  border = NA, 
