...
#' @export 
setClass(Class="report_silver_eel",
...
setValidity("report_silver_eel",function(object)
	{
	  ...
	  label1<-'report_silver_eel should only be for eel (tax_code=2038)'
	  rep2=all(object@stage@data$std_code%in%c('AGG','AGJ'))
	  label2<-'Only stages silver (AGG) and yellow (AGJ) should be used in report_silver_eel'
...
#' @keywords internal
setMethod("charge",signature=signature("report_silver_eel"),definition=function(object,h) {
	  ...
	  if (exists("report_arg_date_debut",envir_stacomi)) {
		object@horodatedebut@horodate<-get("report_arg_date_debut",envir_stacomi)
	  } else {
		funout(gettext("You need to choose the starting date\n",domain="R-stacomiR"),arret=TRUE)
	  }
	  if (exists("report_arg_date_fin",envir_stacomi)) {
		object@horodatefin@horodate<-get("report_arg_date_fin",envir_stacomi)
	  } else {
...
#' @export
setMethod("choice_c",signature=signature("report_silver_eel"),definition=function(object,
		dc,
		taxa=2038,
		stage='AGG',
	  ...
	  r_silver@taxa<-charge_with_filter(object=r_silver@taxa,r_silver@dc@dc_selectionne)			
	  r_silver@taxa<-choice_c(r_silver@taxa,taxa)
	  r_silver@stage<-charge_with_filter(object=r_silver@stage,r_silver@dc@dc_selectionne,r_silver@taxa@data$tax_code)	
	  r_silver@stage<-choice_c(r_silver@stage,stage)
	  r_silver@par<-charge_with_filter(object=r_silver@par,r_silver@dc@dc_selectionne,r_silver@taxa@data$tax_code,r_silver@stage@data$std_code)	
	  r_silver@par<-choice_c(r_silver@par,par,silent=silent)
	  r_silver@horodatedebut<-choice_c(object=r_silver@horodatedebut,
		  nomassign="reportArg_date_debut",
		  funoutlabel=gettext("Beginning date has been chosen\n",domain="R-stacomiR"),
		  horodate=horodatedebut, 
		  silent=silent)
	  r_silver@horodatefin<-choice_c(r_silver@horodatefin,
...
#' @author Cedric Briand \email{cedric.briand"at"eptb-vilaine.fr}
setMethod("calcule",signature=signature("report_silver_eel"),definition=function(object,silent) {
	  r_silver<-object
	  ...
	  for (i in 1:length(lesdc)){
		...
		other<-fun_date_extraction(other,"ope_date_debut",jour_an = TRUE, jour_mois = FALSE)
...
#' @export
setMethod("plot", signature(x = "report_silver_eel", y = "missing"), definition=function(x, plot.type="1", silent=FALSE){ 
...
#' @export
setMethod("summary",signature=signature(object="report_silver_eel"),definition=function(object,silent=FALSE,...){
	  r_silver<-object
	  if (exists("r_silver",envir_stacomi)) {
		r_silver<-get("r_silver",envir_stacomi)
	  } else {      
...
#' @export
setMethod("print",signature=signature("report_silver_eel"),definition=function(x,...){ 
	  sortie1<-"r_silver=new('report_silver_eel')"
	  sortie2<-stringr::str_c("r_silver=choice_c(r_silver,",
