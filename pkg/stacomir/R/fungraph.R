...
fungraph=function(report_mig,tableau,time.sequence,taxa,stage,dc=NULL,silent,color=NULL,color_ope=NULL,...){
#mat <- matrix(1:6,3,2)
#layout(mat)
  #browser() 
  #cat("fungraph")
  ...
  if (is.null(dc)) dc=report_mig@dc@dc_selectionne[1]
  ...
  dis_commentaire=  as.character(report_mig@dc@data$dis_commentaires[report_mig@dc@data$dc%in%dc]) # commentaires sur le DC
  ###################################
  ...
  matplot(time.sequence,cbind(tableau$MESURE+tableau$CALCULE+tableau$EXPERT+tableau$PONCTUEL,
	  ...
	  cex.main=cex.main,
	  font.main=font.main)
  if(report_mig@timestep@step_duration=="86400"){ # pas de temps journalier
  ...
  t_operation_ope<-report_ope@data[report_ope@data$ope_dic_identifiant==dc,]
  ...
  report_df@data<-report_df@data[report_df@data$per_dis_identifiant==df,]
  report_dc@data<-report_dc@data[report_dc@data$per_dis_identifiant==dc,]
  ...
  if (dim(report_df@data)[1]==0 ) {
	...
	if (sum(report_df@data$per_etat_fonctionnement==1)>0){    
	  rect(   xleft =graphdate(as.POSIXct(report_df@data$per_date_debut[
					  report_df@data$per_etat_fonctionnement==1])), 
		  ybottom=2.1,
		  xright=graphdate(as.POSIXct(report_df@data$per_date_fin[
					  report_df@data$per_etat_fonctionnement==1])),
		  ytop=3, 
	...
	if (sum(report_df@data$per_etat_fonctionnement==0)>0){              
	  rect(   xleft =graphdate(as.POSIXct(report_df@data$per_date_debut[
					  report_df@data$per_etat_fonctionnement==0])), 
		  ybottom=2.1,
		  xright=graphdate(as.POSIXct(report_df@data$per_date_fin[
					  report_df@data$per_etat_fonctionnement==0])),
		  ytop=3, 
	...
	listeperiode<-
		fun_table_per_dis(typeperiode=report_df@data$per_tar_code,
  ...
  if (dim(report_dc@data)[1]==0 ) {
	...
	if (sum(report_dc@data$per_etat_fonctionnement==1)>0){ 
	  rect(   xleft =graphdate(as.POSIXct(report_dc@data$per_date_debut[
					  report_dc@data$per_etat_fonctionnement==1])), 
		  ybottom=2.1,
		  xright=graphdate(as.POSIXct(report_dc@data$per_date_fin[
					  report_dc@data$per_etat_fonctionnement==1])),
		  ytop=3, 
		  col = mypalette["working"],
		  border = NA, 
	...
	{ 
	  rect(   xleft =graphdate(as.POSIXct(report_dc@data$per_date_debut[
					  report_dc@data$per_etat_fonctionnement==0])), 
		  ybottom=2.1,
		  xright=graphdate(as.POSIXct(report_dc@data$per_date_fin[
					  report_dc@data$per_etat_fonctionnement==0])),
		  ytop=3, 
		  ...
		  lwd = 1) }
	listeperiode<-
		fun_table_per_dis(typeperiode=report_dc@data$per_tar_code,
