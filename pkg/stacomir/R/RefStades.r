...
#' choice(objet,objetBilan=bilanMigrationPar)
setMethod("choice",signature=signature("RefStades"),definition=function(objet,objetBilan=NULL,is.enabled=TRUE) {
			if (nrow(objet@data) > 0){
				hstd=function(h,...){
					...
					if (!is.null(objetBilan)) {
						...
						# elle est alors affichee des le debut par la methode choice � laquelle on ne passe pas d'objetBilan en parametre 
						...
						if (class(try(objetBilan@par,silent=TRUE))!="try-error") {
							...
							choice(objetBilan@par,is.enabled=TRUE)
						} 
						...
						if (class(try(objetBilan@parqual,silent=TRUE))!="try-error") {
							...
							choice(objetBilan@parqual,label=get("msg",envir=envir_stacomi)$RefStades.3,nomassign="refparqual",frameassign="frame_parqual",is.enabled=TRUE)
						}
#il y a bien un objet parquan dans l'objet Bilan
						if (class(try(objetBilan@parquan,silent=TRUE))!="try-error") {
							...
							choice(objetBilan@parquan,label=get("msg",envir=envir_stacomi)$RefStades.4,nomassign="refparquan",frameassign="frame_parquan",is.enabled=TRUE)
						}
