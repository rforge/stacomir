...
ggplot2usr=function(data,envir=.GlobalEnv){
	#ggplot2usr_libraries()
	if (class(data)!="character") stop("data should be a character")
	options(guiToolkit = "RGtk2")
	build_proto()
	wingg= gwindow("ggplot graphical interface")
	assign("wingg",wingg,.GlobalEnv)
	ggi=Ggploti$new(data=data,window=wingg)
	assign("ggi",ggi,envir=.GlobalEnv)
	ggi$display() 
}
