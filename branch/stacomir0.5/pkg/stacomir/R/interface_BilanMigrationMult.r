...
#TODO addmsg
	...
	# todo addmsg
	toolbarlist = list(
			Calc=gaction(handler=hbilanMigrationMultcalc, action=bilanMigrationMult, icon="new", label="calcul", tooltip=get("msg",envir=envir_stacomi)$interface_BilanMigrationMult.2),
			Graph=gaction(handler=hbilanMigrationMultgraph, icon="graph", label="graph", tooltip=get("msg",envir=envir_stacomi)$interface_BilanMigrationMult.3),
			Graph2=gaction(handler=hbilanMigrationMultgraph2, icon="graph2", label="grcum", tooltip=get("msg",envir=envir_stacomi)$interface_BilanMigrationMult.4),
			Stat=gaction(handler=hTablebilanMigrationMult, icon="dataframe", label="stat", tooltip=get("msg",envir=envir_stacomi)$interface_BilanMigrationMult.5),    
